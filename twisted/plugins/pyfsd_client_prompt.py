...
class FSDClientServiceMaker:
    ...
    def makeService(self, options: FSDClientOptions) -> MultiService:
        ...
        try:
            ...
            print(f"Edit {options['config-path']} and run again.")
            from loguru import logger
            from twisted.internet.reactor import stop, callWhenRunning
            logger.remove()
            callWhenRunning(stop)
            return MultiService()
